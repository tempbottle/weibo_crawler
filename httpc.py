...
class HTTPException(Exception):
    def __init__(self, resp, msg=None):
        ...
        self.msg = msg
    def __str__(self):
        return 'HTTPException: ' + str(self.resp) + str(self.msg)
...
class CurlHTTPC(HTTPC):
    ...
    def post(self, url, body='', headers={}):
        ...
        logger.debug('request body:: ' + shorten(body, 1024))
        return self._curl('curl -X "POST" %s "%s" -T "%s"'% 
                (self._headers2txt(headers), url, tmp_file))
    ...
    def post_multipart(self, url, local_file, filename='file1', fields={}, headers={}):
        ...
        post_info = ' '.join( ['-F "%s=<%s"' % (k,self.content_to_tmpfile(v)) for (k,v) in fields.items()])
        if local_file: 
    ...
    def content_to_tmpfile(self, content):
        tmp_file = self._random_tmp_file('put-body')
        f = file(tmp_file, 'wb')
        f.write(content)
        f.close()
        return tmp_file
    ...
    def _curl(self, cmd, body_file=None):
        try:
            ...
            raise HTTPException(None, 'detail: ' + str(e))
    ...
    def _curl_ll(self, cmd, body_file=None):
        ...
        logger.debug(shorten(resp_body, 1024))
